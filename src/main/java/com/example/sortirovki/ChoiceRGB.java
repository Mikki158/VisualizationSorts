...
public class ChoiceRGB extends Sort {
    ...
    public void MainSort() {
        ...
        stage.setTitle("Сортировка выбором");
        stage.setWidth(1100);
        stage.setHeight(666);
        ...
        for(int i = sizelist - 1; i > 0; i--){
            max = i;
            for(int j = i - 1; j >= 0; j--){
                if(numbers.get(j) > numbers.get(max)){
                    max = j;

                }
            }
            tempizvl = numbers.get(i);
            numbers.set(i, numbers.get(max));
            numbers.set(max, tempizvl);
            operations.add(i);
            operations.add(max);
            operations.add(10);
            lenght += 1;
